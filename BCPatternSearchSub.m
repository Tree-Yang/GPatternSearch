%==========================================================================
%SUBFUNCTION FOR PATTERN SEARCH: Bound constrained pattern search for sub-problems
%-----------------------------------------------
function [x_hist, f_hist, nlc_hist, alf_hist] = BCPatternSearchSub(x0, f0, nlc0, obj_fun, ...
    ...
    while sl >= sl_esp && ite <= ite_max
        ...
        if strcmpi(pattern, 'HJ')
            if ite == 1
                [x, f, nlc, alf, flag]      = polls(x, f, sl, nlc, obj_fun, lb, ub,...
                                                        nlcon_fun, S, lambda_k, mu_k);
            else
                zooms                      = x_hist(ite) - x_hist(ite-1);
                x_tmpc                     = x+zooms;
                [x, f, nlc, alf, flag]     = polls(x_tmpc, f, nlc, sl, obj_fun, lb, ub,...
                                                        nlcon_fun, S, lambda_k, mu_k);
                if flag == 0
                    [x, f, nlc, alf, flag] = polls(x, f, nlc, sl, obj_fun, lb, ub,...
                                                        nlcon_fun, S, lambda_k, mu_k);
                end
            end
